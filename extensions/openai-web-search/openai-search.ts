...
function resolveConfiguredResponsesUrl(value: unknown): string {
	...
	if (typeof value !== "string" || value.trim().length === 0) {
		throw new Error(`openaiResponsesUrl in ${getConfigPath()} must be an absolute HTTPS URL`);
	}
	let url: URL;
	try {
	...
	if (url.protocol !== "https:") {
		throw new Error(`openaiResponsesUrl in ${getConfigPath()} must use HTTPS`);
	}
	return url.toString();
}
...
function resolveConfiguredSearchModel(value: unknown): string | undefined {
	...
	if (typeof value !== "string" || value.trim().length === 0) {
		throw new Error(`openaiSearchModel in ${getConfigPath()} must be a non-empty string`);
	}
	return value.trim();
}
...
async function resolvePiAuth(
	...
	for (const provider of SEARCH_PROVIDERS) {
		...
		try {
			...
			// Try the next OpenAI provider, then the configured/API-key path.
		}
	}
	return undefined;
}
...
async function parseOpenAIResponse(response: Response): Promise<Record<string, unknown>> {
	...
	for (const line of text.split("\n")) {
		...
		try {
			...
			// Ignore unrelated or malformed SSE events; fail below if no output exists.
		}
	}
	...
	for (const item of output) {
		...
		for (const part of content) {
			...
			for (const annotation of annotations) {
				...
				addResult(results, seenUrls, citation.url, citation.title, extractSnippetAround(text, citation.start_index, citation.end_index));
			}
		}
	}
	for (const item of output) {
...
export async function searchWithOpenAI(
	...
	if (!auth) {
		throw new Error(
			...
			`  2. Create ${getConfigPath()} with { "openaiApiKey": "your-key" }\n` +
